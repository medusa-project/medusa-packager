...
tsv.each_with_index do |row, i|
  ...
  builder = Nokogiri::XML::Builder.new do |xml|
    xml['lrp'].Object("xmlns:lrp" => "http://www.library.illinois.edu/lrp/terms#") {
      ...
      xml['lrp'].repositoryId {
        # xml.text(object_id.chomp('.tif').chomp('.jp2'))
        xml.text(r['Repository ID'])
      }

      if r['File Name'].nil?
        children = tsv.find_all{ |child| child['Map No. in Bassett Bibliography'] == r['Map No. in Bassett Bibliography'] && child['File Name'] }
        if children.any?
      ...
      if r['Local Call Number']
        xml['lrp'].callNumber {
          xml.text(r['Local Call Number'])
        }
      end
      ...
      if r['Keyword']
        r['Keyword'].split(';').each do |keyword|
          xml['lrp'].keyword {
            xml.text(keyword.strip)
          }
        end
      end
      if r['Language']
        r['Language'].split(';').each do |language|
          xml['lrp'].language {
            xml.text(language.strip)
          }
        end
      end
      ...
      if r['Map No. in Bassett Bibliography']
        xml['lrp'].localId {
          xml.text(r['Map No. in Bassett Bibliography'])
        }
      end


      if r['Color']
        xml['lrp'].materialsColor {
          xml.text(r['Color'])
        }
      end

      if r['Technique']
        xml['lrp'].materialsTechniques {
          xml.text(r['Technique'])
        }
      end
      ...
      if r['Collection Publisher']
        xml['lrp'].source{
          xml.text(r['Collection Publisher'])
        }
      end
      ...
      unless r['File Name'].nil?
        ...
        xml['lrp'].accessMasterPathname {
          xml.text("/#{r['Local Bib ID']}/access/#{r['File Name']}".chomp('.jp2').chomp('.tif') + '.jp2')
        }
      end
      ...
      unless r['File Name'].nil?
        ...
        xml['lrp'].preservationMasterPathname {
          xml.text("/#{r['Local Bib ID']}/preservation/#{r['File Name']}".chomp('.jp2').chomp('.tif') + '.tif')
        }
      end
  ...
  filename = (r['File Name'].nil?) ? r['Repository ID'] : r['File Name']
  filename = filename.gsub('.jp2', '')
  dest_pathname = "#{dest}/#{filename}.xml"
